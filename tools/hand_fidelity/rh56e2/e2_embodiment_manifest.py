...
def build(asset_manifest, donor, audit=None):
    ...
    m['source_asset'] = {'asset_id': a['asset_id'], 'kind': 'public_exact_model_prior', 'exact_hand_model': True, 'installed_calibration': 'INCOMPLETE', 'urdf_sha256': a['merged_urdf']['sha256'], 'urdf_name': a['merged_urdf']['name'],
                         ...
                         'collision_model': ({'kind': 'e2_r5_folded_slabs', 'cooking': 'e2_r5:fixed_hand_links_folded;palm=e2_palm_yz_slabs_v2_thumb_cavity(4mm_xz_exact_meshes+1mm_thumb_cavity_carve_1.5mm_margin);fingers=convexDecomposition(importer);offsets=importer_default;baked_into_urdf'} if a.get('representation_r4', {}).get('collision_model_kind') == 'e2_r5_folded_slabs'
                                             else {'kind': 'e2_r4_folded_slabs', 'cooking': 'e2_r4:fixed_hand_links_folded;palm=e2_palm_yz_slabs_v1(4mm_xz_exact_meshes);fingers=convexDecomposition(importer);offsets=importer_default;baked_into_urdf'} if 'representation_r4' in a
