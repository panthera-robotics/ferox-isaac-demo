"""Hand-agnostic variant of hand_grasp.py (RH56E2 acceptance lane B): the bench hand is named by the probe config `hand` block
(inspire_grasp_bench.HandBench); absent, the frozen donor path is byte-identical. Grasp/rack configs stay in donor terms and are mapped
closure-preservingly to the bench hand; the wrist fixture mounts the bench root with the declared rotation so holder/rack/gravity are identical.
...
def main():
    ...
    from inspire_grasp_bench import HandBench,DONOR_JOINTS,DONOR_UPPER   # lane B: hand-agnostic bench binding (donor defaults = byte-identical path)
    ...
    bench=HandBench.from_config(config_data);config_data={k:v for k,v in config_data.items() if k!='hand'}   # grasp/rack configs stay in DONOR terms
    ...
    scope={'holder_attachment_active':False,'holder_fixture_support_active':False,'wrist_fixed_support':True,
           ...
           'source':'provisional_FTP_right_donor_not_verified_RH56E2_bytes' if bench.is_donor else bench.label,'bench':bench.facts(),
    ...
    write('grasp_config.json',{'config':asdict(cfg),'config_sha256':cfg.sha256,'scope':scope,'mode':mode,'hand_bench':bench.facts()})
    ...
    try:
        ...
        DONOR_LIMITS={DONOR_JOINTS[a]:[0.,DONOR_UPPER[a]] for a in DONOR_JOINTS}   # closed targets are formed in donor space (= the bench URDF limits on the donor)
        initial_q=bench.map_targets(cfg.initial_targets());target_q=bench.map_targets(cfg.closed_targets(DONOR_LIMITS))   # donor-space closure targets, mapped to the bench hand
        assert len(independent)==6 and len(mimics)==6 and set(independent)==set(initial_q),(independent,sorted(initial_q))
        def position(name):
        ...
        fixture=write_wrist_fixture(source,out/'supported_wrist.urdf',root_link=bench.root_link,mount_rpy=bench.mount_rpy)
        ...
        authored_links={l.get('name') for l in root.findall('link') if l.find('inertial/mass') is not None}
        mass_authored=sum(float(UsdPhysics.MassAPI(p).GetMassAttr().Get()) for p in stage.Traverse() if p.HasAPI(UsdPhysics.MassAPI) and UsdPhysics.MassAPI(p).GetMassAttr().Get() is not None and (p.GetName() in authored_links or p.GetName().startswith('fixture_')))
        assert abs((mass if bench.is_donor else mass_authored)-source_mass-fixture['fixture_added_mass_kg'])<.001,(mass,mass_authored,source_mass)
        instances=set()
        ...
        if bench.is_donor:assert len(wrappers)==30
        collision_wrapper_count=len(wrappers)
        ...
        if bench.palm_collision=='donor_slabs_v2':
            ...
            candidate={'candidate_id':'mesh_as_delivered','statement':'no palm replacement: the delivered mesh colliders (convex decomposition) are used as-is','collision_wrapper_count':collision_wrapper_count,'palm_candidate_id_in_config_ignored':cfg.palm_candidate_id}
        ...
        for name,t in initial_fk.items():
            p=stage.GetPrimAtPath('/'+robot_prim+'/'+name);assert p,('/'+robot_prim+'/'+name)
        ...
        views={k:SimulationManager.get_physics_sim_view().create_rigid_body_view(p) for k,p in
               [('palm','/World/Hand/'+bench.root_link),('holder',marker['holder_body']),('nib',marker['nib_body'])]}
        ...
        donor_of={bench.bench_name(dn):dn for dn in DONOR_JOINTS.values()}   # bench joint -> donor joint (identity on the donor)
        ...
        for sequence in range(total_steps):
            ...
            command=[initial_q[n]+fraction*(target_q[n]-initial_q[n])-(opening*rack_cfg.release_opening_rad*bench.scale(donor_of[n]) if (cycle_mode and n in opening_names) else 0.) for n in independent]
            if cycle_mode:
                ...
                for name,forward in ((bench.axis_joints['thumb_rotation'],bench.map_value('right_thumb_1_joint',rack_cfg.release_thumb_yaw_rad)),(bench.axis_joints['thumb_bend'],bench.map_value('right_thumb_2_joint',rack_cfg.release_thumb_bend_rad))):
