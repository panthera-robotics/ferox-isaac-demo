...
| **Grasp** | **REACHABLE-BUT-PINCHES (2026-08-23).** Pre-grasp is now reachable — pulling the IK null space toward **wrist mid-range** cleared the `right_wrist_pitch`/`yaw` pinning, and closure is repeatable. The enclosure test finally ran and is decisive: **6 finger links in contact at 43.8 N, 0 within 45 mm of the object axis, 0 below its centre.** The hand **pinches**, it does not cage — which is why force, friction, contact count, thumb opposition, lift vector, lift rate and grip maintenance were each fixed without ever producing a lift. **Next lever: aperture / approach pose so the object sits between finger segments, not at their tips.** Beware: `ik_wrist_null_gain` 4.0 creates a null-space equilibrium (identical arm pose, residual frozen ~59 mm); 1.5 is correct. |
