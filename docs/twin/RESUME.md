...
| **Manipulation** | **PARKED — KINEMATICALLY CHARACTERIZED (2026-08-23). Do NOT tune it further.** **START AT `docs/mm/MANIPULATION_VERDICT.md`** — envelope table, the two constants, and the three viable next paths (gripper swap / pinch mode / real-G1 data) with what each requires. Run-by-run evidence in `docs/mm/evidence/MM5/ENVELOPE_VERDICT.md`. Two hard numbers: **Dex5 closed fingertip spread 21.5 mm** and **palm vertical ceiling 0.946 m max / 0.935 m median** (`pelvis_z` 0.80, fixed base). The reachable band and the graspable band **do not overlap**: 50–66 mm objects are reachable at ~0.95 m but exceed the tip spread (43.83 N across 6 links, **0 links within 45 mm of the object axis, 0 below its centre** → pinch, never a cage); a 30 mm block is reachable only in a narrow window near 0.95 m, and closure **topples it (36.9°)** rather than caging it. Size and height are **coupled** — a smaller object sits lower, and lower is further from a shoulder already at its limit, so shrinking the object to fit the hand moves it out of reach. **What it needs: a graspable object (feature ≤21.5 mm at the contact band), a pinch-grasp primitive for small items, or a gripper — NOT more tuning.** Separately real: the DLS IK has **no obstacle term** and will drive the hand through furniture (measured: palm stalled 45–54 mm short of a riser face, 9/10 trials). |
| ~~Grasp (superseded)~~ | **DESCENT / IK-LIMITED — not grip.** Half the timeouts pin `right_wrist_pitch`/`right_wrist_yaw` at their stops (`IK_INFEASIBLE`), the rest stall 43–70 mm without pinning anything (`SERVO_SLOW`, unchanged at a 45 s window). The commanded **palm orientation** at counter height is not achievable by this wrist. |
...
| **Instruments** | `scripts/mm5_preflight.sh` is **mandatory before any grasp number**. KEEP the **enclosure logger** (links within N mm of the object axis / below its centre — it is what turned "43.83 N of contact" into "pinch, not cage") and the **IK classifier** (`IK_INFEASIBLE` names the pinned joints and their limits; `SERVO_SLOW` means converging with nothing at a stop — this is what separates a kinematic limit from a timeout). Five instrument defects in one session produced five confident wrong numbers; none were robot defects. No capability may be claimed on an unverified gauge. |
...
| **C-39 — CLOSED AND PARKED (Mohammed, 2026-08-22). MM4 = OMNI-HOLD.** | The decisive A/B is done: the **reference MuJoCo body, imported unmodified through the MJCF importer, falls in our simulator exactly as ours does** (`pitch +86.3°` vs `+88.3°`, base 0.10 m, ~2.5 s after release) while the same body **stands** under the same deploy binary in the reference MuJoCo sim. Nine bisect runs — solver iterations, friction combine, contact/rest offset, depenetration velocity, self-collision, the reference's own 200 Hz, implicit drives — **all FALLS**, all landing in the same place. So it is not the asset, the wire, the hands, SONIC, or any single PhysX setting; what remains is the Isaac harness against MuJoCo as a whole, and **that comparison is explicitly NOT funded**. **Decision: the omni locomotion policy is the twin\'s balancer** — it works, and it is on film walking **6.159 m against 6.0 m commanded**. **SONIC is hardware-only**, its finetune deferred to the Spark. **MM4\'s balancing requirement is met by omni-hold, not SONIC.** Read `docs/mm/evidence/C39/{AB_ASSET_VERDICT,BISECT_VERDICT}.md`. |
| ~~**C-39 — SONIC IS OUT OF THE LOOP. PINGED AGAIN.**~~ (superseded by the row above) | The static discriminator settles it: PD-hold the nominal stance with the base free and **nothing running at all** — no SONIC, no bridge, no lowcmd — and it still falls in 2 s. Implicit PhysX drives fall identically, so the force path is not it either. **The arithmetic:** mass 39.005 kg, CoM **5.4 cm ahead of the ankles** → 20.7 Nm of ankle torque needed → at kp 40 that is **0.52 rad of deflection, which is the joint's entire travel**. No joint-space PD at these gains can hold this stance. **Read `docs/mm/evidence/MM4/C39_FORCEPATH.md`.** Open: the twin's foot collider vs MuJoCo's four 5 mm spheres (toe 0.12 m ahead of the ankle). NOTE `get_net_contact_forces` is absent in this build — the foot forces are joint reactions, not contact. |
