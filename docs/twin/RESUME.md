...
| **C-39 — SONIC SEES THE FALL. PINGED.** | Observability is proven, not assumed: the IMU wire tracks a commanded 18° tilt to 0.003°; SONIC's OWN logged state (`--enable-csv-logs`) runs 0→89.09° against ground truth's 0→88.63° with **zero measured lag**; and a 15° tilt moves its commanded posture by 1.15 rad, so the loop is closed. Then the **bare 29-DoF robot with no hands falls too** — so the hands are not the wall. Per the decision matrix this is the PING branch. **Read `docs/mm/evidence/MM4/C39_OBSERVABILITY.md`; traces in `MM4/obs/`.** Note `g1_29dof_trainhands.usd` is NOT bare — it references the locked-hands Dex5 asset; `HAND=none` is the bare robot. |
