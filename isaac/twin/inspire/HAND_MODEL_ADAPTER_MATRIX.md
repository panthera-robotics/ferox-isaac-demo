...
| D | **UnifoLM-VLA-0** | — | — | — | — | — | **PENDING_SOURCE** — the model lane owns the one bounded code-only fetch (checkpoint recorded as a single 18.98 GB pickle: over the download cap, no-pickle rule; no weights); this row is filled only from the fetched source (joint names, order, units, normalization), never from a card or a guess | none |
